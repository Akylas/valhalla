...
void serialize_edges(const Location& location,
  ...
  auto serialize_edge = [&](const PathEdge& edge) {
    ...
    try {
      ...
      // CARTOHACK: mbtiles sets can be missing a tile
      if (!tile) {
...
void serialize_nodes(const Location& location,
  ...
  for (const auto& e : location.correlation().edges()) {
    if (e.end_node()) {
      // CARTOHACK: mbtiles sets can be missing a tile
      auto tile = reader.GetGraphTile(GraphId(e.graph_id()));
      if (!tile) {
        throw std::runtime_error("Missing tile");
      }
      nodes.emplace(tile->directededge(GraphId(e.graph_id()))->endnode());
    }
  }
